...
class ServiceRecordForm(ConfigurableFormMixin, forms.ModelForm):
    form_key = "service"
    date = forms.DateField(widget=forms.DateInput(attrs={"type": "date"}))

    class Meta:
        model = ServiceRecord
        fields = ("date", "mileage", "cost", "notes")

    def __init__(self, *args, **kwargs):
        self._garage = kwargs.pop("garage", None)
        super().__init__(*args, **kwargs)

        labels_hu = {"date": "Dátum", "mileage": "Kilométeróra állás", "cost": "Költség", "notes": "Megjegyzés"}
        ...
        for name, field in self.fields.items():
            ...
            if isinstance(widget, forms.Select):
                ...
                rows = 2 if name == "notes" else 3
                widget.attrs.update({"class": "form-control", "rows": rows})
            else:
...
class BaseServiceRecordItemFormSet(forms.BaseInlineFormSet):
    def __init__(self, *args, **kwargs):
    ...
    def clean(self):
        ...
        if not non_deleted:
            raise forms.ValidationError("Legalább egy szervizsort adjon meg.")


ServiceRecordItemFormSet = inlineformset_factory(
    ServiceRecord,
    ServiceRecordItem,
    form=ServiceRecordItemForm,
    formset=BaseServiceRecordItemFormSet,
    extra=1,
    can_delete=True,
)
